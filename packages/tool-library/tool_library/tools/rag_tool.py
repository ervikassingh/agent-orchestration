...
class RAGTool(BaseTool):
    ...
    async def run(self, input_data: dict[str, Any]) -> ToolResult:
        ...
        if not query:
            return ToolResult(
                self.config.name, None, success=False, error="Missing required parameter: 'query'"
            )
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout_seconds) as client:
                response = await client.post(
                    "http://localhost:8000/rag/query", json={"question": query}
                )
            ...
            return ToolResult(
                self.config.name,
                {"answer": "I could not connect to the knowledge base.", "source": "fallback"},
            ...
            return ToolResult(
                self.config.name, None, success=False, error=f"RAG query failed: {exc}"
            )
