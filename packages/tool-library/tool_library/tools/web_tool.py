...
class WebSurfTool(BaseTool):
    ...
    def __init__(self) -> None:
        super().__init__(
            ToolConfig(
                name="web_surf", description="Fetch content from a URL and return the readable text"
            )
        )
    ...
    async def run(self, input_data: dict[str, Any]) -> ToolResult:
        ...
        if not url:
            return ToolResult(
                self.config.name, None, success=False, error="Missing required parameter: 'url'"
            )
        ...
        try:
            ...
            return ToolResult(
                self.config.name, None, success=False, error=f"Request to {url} timed out"
            )
        ...
        return ToolResult(
            self.config.name,
            {"content": text, "source": url, "length": len(text)},
